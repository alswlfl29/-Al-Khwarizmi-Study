/**
 * 폰켓몬
 * 문제 설명: 당신은 폰켓몬을 잡기 위한 오랜 여행 끝에, 홍 박사님의 연구실에 도착했습니다. 홍 박사님은 당신에게 자신의 연구실에 있는 총 N 마리의 폰켓몬 중에서 N/2마리를 가져가도 좋다고 했습니다.
...
function solution(nums) {
  var answer = 0;
  const choiceCount = nums.length / 2; // 선택할 수 있는 폰켓몬 수
  // 1. 배열 nums를 집합(중복 제거)으로 만들기
  let set = new Set(nums);
  // 2. 만약 선택할 수 있는 폰켓몬 수보다 집합의 크기가 작을 경우 집합 크기 반환하고 그렇지 않다면 선택할 수 있는 폰켓몬 수 반환
  if (set.size < choiceCount) return set.size;
  return choiceCount;
}
